...
# Outputs: Console terminal UI, execution run logs in logs/, structured exit codes
# ============================================================================
...
Options:
  ...
  -d, --dry-run             Run pre-checks and validation without mutating cluster state
      ...
      --phase <NN>          Run single phase in isolation (e.g. 01, 02, 03, 05, 06)
      --clean, --clear      Purge generated logs, output reports, and snapshots (manual trigger)
  ...
  -r, --resume              Resume upgrade from last completed hop detected in logs
      ...
      --skip-operator-upgrade Skip operator InstallPlan mutations; run compatibility and validation checks only
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --clean|--clear|--clear-artifacts)
            ...
            ;;
        --phase)
            [[ -n "${2:-}" ]] || { msg_err "Flag '$1' requires a phase number (e.g. 01, 02, 03, 05, 06)."; exit 1; }
            PHASE_ARG="$2"
            if [[ "$PHASE_ARG" =~ ^[0-9]+$ ]]; then
                printf -v TARGET_PHASE "%02d" "$PHASE_ARG"
            else
...
# Normalize phase flags if --dry-run was passed alongside isolated --phase
        ...
        case "$MODE_CHOICE" in
            ...
            5)
                ...
                EXEC_TYPE_SUB_OPTIONS=(
                    ...
                    "Dry Run Mode (Read-only validation without cluster mutations)"
                )
                ...
                if (( ISOLATED_TYPE_CHOICE == 1 )); then
                    ...
                    case "$TARGET_ISOLATED_PHASE" in
                        02) AUTO_REMEDIATION_MODE="true" ;;
                        06) SKIP_OPERATOR_UPGRADE=false ;;
                    esac
                    ...
                    case "$TARGET_ISOLATED_PHASE" in
                        01) DRY_RUN=true ;;
                        02) DRY_RUN=true; AUTO_REMEDIATION_MODE="false" ;;
                        03) DRY_RUN=true ;;
                        05) DRY_RUN=true ;;
                        06) DRY_RUN=false; SKIP_OPERATOR_UPGRADE=true ;;
                    esac
...
' "$VARS_JSON" "$TARGET_CLUSTER")
        ...
        if [[ ("$SKIP_TO_PHASE" == "05" && "$STOP_AFTER_PHASE" == "05") || ("$SKIP_TO_PHASE" == "06" && "$STOP_AFTER_PHASE" == "06") ]]; then
            PATH_OPTIONS=(
...
if [[ "$DRY_RUN" == "true" || "${STOP_AFTER_PHASE:-}" == "01" || "${STOP_AFTER_PHASE:-}" == "02" || "${STOP_AFTER_PHASE:-}" == "05" || "${SKIP_OPERATOR_UPGRADE:-false}" == "true" ]]; then
...
# ----------------------------------------------------------------------------
# 11. Playbook Target Resolution & Extra-Vars Serialization
# ----------------------------------------------------------------------------
# Determine target playbook: dispatch isolated phase playbook directly, or main.yml for multi-phase orchestrator flows
TARGET_PLAYBOOK="${BASE_DIR}/main.yml"
IS_STANDALONE_PHASE=false

RESOLVED_PHASE=""
if [[ -n "${TARGET_ISOLATED_PHASE:-}" ]]; then
    RESOLVED_PHASE="${TARGET_ISOLATED_PHASE}"
elif [[ -n "${SKIP_TO_PHASE:-}" && "${SKIP_TO_PHASE}" == "${STOP_AFTER_PHASE:-}" ]]; then
    RESOLVED_PHASE="${SKIP_TO_PHASE}"
fi

if [[ -n "$RESOLVED_PHASE" ]]; then
    case "$RESOLVED_PHASE" in
        "01"|1)
            TARGET_PLAYBOOK="${BASE_DIR}/01_Policy_Check.yaml"
            IS_STANDALONE_PHASE=true
            ;;
        "02"|2)
            TARGET_PLAYBOOK="${BASE_DIR}/02_Pre_upgrade_check.yaml"
            IS_STANDALONE_PHASE=true
            ;;
        "03"|3)
            TARGET_PLAYBOOK="${BASE_DIR}/03_Initiate_upgrade.yaml"
            IS_STANDALONE_PHASE=true
            ;;
        "05"|5)
            TARGET_PLAYBOOK="${BASE_DIR}/05_post_Upgrade_Checks.yaml"
            IS_STANDALONE_PHASE=true
            ;;
        "06"|6)
            TARGET_PLAYBOOK="${BASE_DIR}/06_Operator_Upgrade.yaml"
            IS_STANDALONE_PHASE=true
            ;;
    esac
fi
...
skip_operator_upgrade = sys.argv[9].lower() == "true" if len(sys.argv) > 9 else False
run_timestamp = sys.argv[10] if len(sys.argv) > 10 else ""
standalone_phase = sys.argv[11].lower() == "true" if len(sys.argv) > 11 else False
...
if mail_to_cli:
    ...
    if recipients:
        ...
        payload["dry_run_mail_to"] = recipients

if skip_cgroup_check:
    payload["skip_cgroup_check"] = True

if skip_operator_upgrade:
    payload["skip_operator_upgrade"] = True

print(json.dumps(payload))
' "$TARGET_CLUSTER" "$HOPS_CSV" "$DRY_RUN" "${SKIP_TO_PHASE:-}" "$SKIP_CGROUP_CHECK" "${STOP_AFTER_PHASE:-}" "${AUTO_REMEDIATION_MODE:-}" "${MAIL_TO_CLI:-}" "$SKIP_OPERATOR_UPGRADE" "$TIMESTAMP" "$IS_STANDALONE_PHASE")
...
if command -v ansible-playbook >/dev/null 2>&1; then
    ansible-playbook "${TARGET_PLAYBOOK}" -e "$EXTRA_VARS" "${ANSIBLE_ARGS[@]}" 2>&1 | tee -a "$LOG_FILE"
...
if (( PLAYBOOK_RC == 0 )); then
    ...
    elif grep -qiE "(Phase 06.*failed|operator_upgrade.*failed|operator_validate.*failed|operator.*failed)" "$LOG_FILE" 2>/dev/null; then
        FINAL_EXIT_CODE=35
        msg_err "Execution halted: Operator upgrade or validation gate failed (Phase 06)."
