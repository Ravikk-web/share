...
# Role / Playbook Dependencies: bash 4.2+, standard POSIX utilities (sed, wc, tput, date)
# Gate Type: N/A (CLI Terminal UI Helper Library)
# Outputs: ANSI/ASCII terminal presentation primitives, interactive menus, status loggers
# ============================================================================
...
# ----------------------------------------------------------------------------
# CLI Help & Guidance Banner
# ----------------------------------------------------------------------------
print_help_banner() {
    draw_box_header "CLI Guidance & Help"
    draw_box_row "Tip: Run './00_Run.sh --help' (or -h) to view all CLI options & flags."
    draw_box_row "Supports non-interactive automation, dry-run, and phase isolation."
    draw_box_footer
    printf "\n"
}
...
render_menu() {
    ...
    while true; do
        ...
        if [[ "$choice" =~ ^[0-9]+$ ]] && (( choice >= 1 && choice <= num_options )); then
            MENU_CHOICE="$choice"
            MENU_VALUE="${options[$((choice-1))]}"
            MENU_SELECTED_INDEX="$choice"
            MENU_SELECTED_VALUE="${options[$((choice-1))]}"
            export MENU_CHOICE MENU_VALUE MENU_SELECTED_INDEX MENU_SELECTED_VALUE
            return 0
        else
...
render_post_run_summary() {
    ...
    if [[ "$policy_report" != "N/A" ]]; then
        local pol_stat="${C_DIM}(Not Generated)${C_RESET}"
        [[ -f "$policy_report" || -f "${BASE_DIR:-.}/${policy_report}" ]] && pol_stat="${C_GREEN_BOLD}(Generated ✔)${C_RESET}"
        draw_box_row "  • Policy Check HTML Report ${pol_stat}:"
        draw_box_row "    ${C_CYAN}${policy_report}${C_RESET}"
    fi
...
# ----------------------------------------------------------------------------
# Operational Artifact Purge (logs/, output/, snapshots/)
# ----------------------------------------------------------------------------
# Purges generated run logs, HTML reports, and JSON snapshots while strictly
# preserving repository tracking files (.gitkeep).
purge_artifacts() {
    local base="${1:-.}"
    local log_dir="${base}/logs"
    local out_dir="${base}/output"
    local snap_dir="${base}/snapshots"
    local root_out_dir="${base}/../output"
    ...
    draw_box_header "Purging Operational Artifacts"
    
    # 1. Clean logs/
    if [[ -d "$log_dir" ]]; then
        log_count=$(find "$log_dir" -maxdepth 1 -type f ! -name ".*" 2>/dev/null | wc -l | tr -d ' ')
        find "$log_dir" -maxdepth 1 -type f ! -name ".*" -exec rm -f {} + 2>/dev/null || true
        draw_box_row "  • Logs directory       : Purged ${log_count} file(s) (preserved .gitkeep)"
    fi
    
    # 2. Clean output/
    if [[ -d "$out_dir" ]]; then
        out_count=$(find "$out_dir" -maxdepth 1 -type f ! -name ".*" 2>/dev/null | wc -l | tr -d ' ')
        find "$out_dir" -maxdepth 1 -type f ! -name ".*" -exec rm -f {} + 2>/dev/null || true
        draw_box_row "  • Output reports       : Purged ${out_count} file(s) (preserved .gitkeep)"
    fi
    if [[ -d "$root_out_dir" ]]; then
        find "$root_out_dir" -maxdepth 1 -type f ! -name ".*" -exec rm -f {} + 2>/dev/null || true
    fi
    
    # 3. Clean snapshots/
    if [[ -d "$snap_dir" ]]; then
        snap_count=$(find "$snap_dir" -maxdepth 1 -type f ! -name ".*" 2>/dev/null | wc -l | tr -d ' ')
        find "$snap_dir" -maxdepth 1 -type f ! -name ".*" -exec rm -f {} + 2>/dev/null || true
        draw_box_row "  • Snapshots directory  : Purged ${snap_count} file(s) (preserved .gitkeep)"
    fi
    
    # 4. Clean any orphaned temporary kubeconfig files in playbooks root
    kube_count=$(find "$base" -maxdepth 1 -name ".kubeconfig-*" -type f 2>/dev/null | wc -l | tr -d ' ')
    if (( kube_count > 0 )); then
        find "$base" -maxdepth 1 -name ".kubeconfig-*" -type f -exec rm -f {} + 2>/dev/null || true
        draw_box_row "  • Ephemeral sessions   : Purged ${kube_count} session kubeconfig file(s)"
    fi
    
    draw_box_footer
    
    local total=$(( log_count + out_count + snap_count + kube_count ))
    msg_ok "Operational artifacts cleanup complete: ${total} total artifact(s) removed."
}
